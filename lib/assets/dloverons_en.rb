...
require "nokogiri"
require "open-uri"
def overons_en(concert,doc,subtitleen)
longurl=doc.css('link[hreflang="en"]')[0].attributes['href'].value
`wget "#{longurl}" -O thisname_en_1.html`
@doc_en=Nokogiri::HTML(File.read("./thisname_en_1.html"))
@doc1_en=@doc_en.css('.page-header')
@doc2_en=@doc_en.css('.single-page-content')
concert.url_en=longurl
concert.title_en=@doc_en.css('h1').text.strip.squish
concert.subtitle_en=subtitleen[longurl.split('/').without('').last]
